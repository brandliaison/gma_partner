...
class TutorialVideoCategoryController extends Controller
{
    /**
    ...
    public function store(Request $request)
    {
        ...
        $category = TutorialVideoCategory::create([
            'name' => $request->name,
        ...
        if ($category) {
            EntityRevision::create([
                'entity_type' => 'TutorialVideoCategory',
    ...
    {
        $category = TutorialVideoCategory::select('_id', 'name', 'slug', 'description', 'title', 'created_at')->find($id);
        ...
        return response()->json($oldcategory = TutorialVideoCategory::find($id), 200);
    }
