...
export default function EditNotifications() {
    ...
    const handleSubmit = (e) => {
        ...
        data.append("notification_category_id", formData.notification_category_id);
        ...
        // API Call (Optional)
        apiClient.post(`/notifications/${id}`, data, {
            headers: { "Content-Type": "multipart/form-data" },
        })
        .then(response => {
            console.log("Success:", response.data);
            navigate('/service-partner/notifications')
            UIkit.notification({
                    ...
                    <div>
                        <div className="uk-card">
                            <div className="uk-card-body">
                                <h5 className="uk-heading-line"><span>Add Notification</span></h5>
                                <form onSubmit={handleSubmit}>
                                    <fieldset className="uk-fieldset">
                                        <div className="uk-grid uk-grid-small uk-child-width-1-2@l" uk-grid="true">
                                            <div>
                                                <input className="uk-input uk-margin-bottom" type="text" name="name" onChange={handleChange} value={formData.name} placeholder="Notification Name" data-sc-input />
                                                <input className="uk-input uk-margin-bottom" type="file" name="image_url" onChange={handleFileChange} data-sc-input />
                                            </div>
                                            <div>
                                                <select className="uk-select uk-margin-bottom" name='notification_category_id' onChange={handleChange} value={formData.notification_category_id} style={{borderTop: 'none', borderLeft: 'none', borderRight: 'none', borderRadius: '0px'}}>
